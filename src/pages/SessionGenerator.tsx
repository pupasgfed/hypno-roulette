  ...
  return (
    <div className="min-h-screen bg-brand-dark">
      <div className="max-w-6xl mx-auto px-4 py-12">
        <h1 className="text-4xl font-bold text-brand-accent mb-12">Générer une Session Complète</h1>
          ...
          <div className="animate-fadeIn space-y-8">
            <div className="bg-brand-mid rounded-lg p-8">
              ...
              <div className="space-y-3">
                {session.map(item => (
                  <div key={item.suggestion_id} className="flex items-center gap-3">
                    <div
                    ...
                    <span className="text-gray-200">{item.suggestion_label}</span>
                  </div>
                ))}
              </div>
            </div>
            ...
            <div className="bg-brand-mid rounded-lg p-8">
              <h2 className="text-2xl font-bold text-brand-accent mb-6">Guide Détaillé</h2>
              <div className="space-y-8">
                {session.map((item, index) => (
                  <div key={item.suggestion_id} className="border-l-4 border-brand-accent pl-6">
                    <div className="flex items-start gap-4 mb-4">
                      ...
                      <div>
                        <h3 className="text-xl font-bold text-brand-accent">{item.suggestion_label}</h3>
                        <div className={`text-sm font-semibold mt-1 ${levelColors[item.suggestion_level]}`}>
                          {levelLabels[item.suggestion_level]}
                        </div>
                      </div>
                    </div>
                    <div className="prose prose-invert max-w-none text-gray-200">
                      <ReactMarkdown>{item.suggestion_description}</ReactMarkdown>
                    </div>
                  </div>
                ))}
              </div>
