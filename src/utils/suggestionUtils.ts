...
export const getRandomSuggestion = (
  ...
  const available = getSimpleSuggestions(excludedIds, filterThemes, excludeNsfw);
...
export const generateSession = (theme: string | null = null, excludeNsfw: boolean = false): SessionItem[] => {
  const session: SessionItem[] = [];
  const usedIds = new Set<string>();

  const getFilteredByLevel = (level: 0 | 1 | 2 | 3 | 4, count: number): Suggestion[] => {
    const filtered = suggestions.filter(s => {
      ...
      if (excludeNsfw && (s.suggestion_nsfw || s.suggestion_level === 4)) return false;
      return true;
    });
    ...
    const allOthers = suggestions.filter(s => {
      if (usedIds.has(s.suggestion_id)) return false;
      if (filtered.some(f => f.suggestion_id === s.suggestion_id)) return false;
      if (excludeNsfw && (s.suggestion_nsfw || s.suggestion_level === 4)) return false;
      return true;
    });
  ...
  // Level 4: 1 suggestion
  const level4 = getFilteredByLevel(4, 1);
  level4.forEach(s => {
    session.push({
      suggestion_id: s.suggestion_id,
      suggestion_label: s.suggestion_label,
